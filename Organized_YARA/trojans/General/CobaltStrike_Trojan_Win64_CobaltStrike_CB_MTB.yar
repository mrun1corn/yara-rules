...
rule Trojan_Win64_CobaltStrike_CB_MTB{
	...
	strings :
		$a_03_0 = {48 89 c3 31 c0 39 c6 7e ?? 48 89 c2 83 e2 ?? 8a 54 15 ?? 32 14 07 88 14 03 48 ff c0 eb } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win64_CobaltStrike_CB_MTB_2{
	...
	strings :
		$a_03_0 = {0f 57 c2 0f 57 ca f3 0f 7f 40 ?? f3 0f 6f 44 10 ?? f3 0f 7f 48 ?? 0f 57 c2 f3 0f 6f 4c 08 ?? f3 0f 7f 40 ?? 0f 57 ca f3 0f 7f 48 ?? 49 83 e9 01 75 } //3
	condition:
		((#a_03_0  & 1)*3) >=3
...
rule Trojan_Win64_CobaltStrike_CB_MTB_3{
	...
	strings :
		$a_03_0 = {03 c8 8b 43 ?? 31 8b ?? ?? ?? ?? 8b 8b ?? ?? ?? ?? 2b 4b ?? 81 f1 ?? ?? ?? ?? 0f af c1 48 8b 8b ?? ?? ?? ?? 89 43 ?? 8b 43 ?? 31 04 11 48 83 c2 ?? 8b 83 ?? ?? ?? ?? 01 43 ?? 48 81 fa ?? ?? ?? ?? 7c } //1
	condition:
		((#a_03_0  & 1)*1) >=1
