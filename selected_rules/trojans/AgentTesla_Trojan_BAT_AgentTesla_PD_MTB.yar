...
rule Trojan_BAT_AgentTesla_PD_MTB_2{
	...
	strings :
		$a_03_0 = {02 16 9a 28 ?? ?? ?? ?? 0a 02 17 9a 75 ?? ?? ?? ?? 06 6f ?? ?? ?? ?? 28 ?? ?? ?? ?? 06 7e ?? ?? ?? ?? 6f ?? ?? ?? ?? 5d 0b 7e ?? ?? ?? ?? 0c 08 07 6f ?? ?? ?? ?? 61 8c ?? ?? ?? ?? 2a } //1
		$a_03_1 = {16 0a 2b 35 00 28 ?? ?? ?? ?? 73 ?? ?? ?? ?? 72 ?? ?? ?? ?? 28 ?? ?? ?? ?? 6f 1f 00 00 0a 16 6a 28 ?? ?? ?? ?? 28 ?? ?? ?? ?? de 0d 26 17 0a 28 ?? ?? ?? ?? de 03 } //1
