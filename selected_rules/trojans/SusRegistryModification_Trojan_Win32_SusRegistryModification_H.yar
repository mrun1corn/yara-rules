
rule Trojan_Win32_SusRegistryModification_H{
	meta:
		description = "Trojan:Win32/SusRegistryModification.H,SIGNATURE_TYPE_CMDHSTR_EXT,05 00 05 00 06 00 00 "
		
	strings :
		$a_80_0 = {72 65 67 2e 65 78 65 20 61 64 64 } //reg.exe add  1
		$a_80_1 = {48 4b 4c 4d 5c 53 59 53 54 45 4d 5c 43 75 72 72 65 6e 74 43 6f 6e 74 72 6f 6c 53 65 74 5c 53 65 72 76 69 63 65 73 5c 4d 50 53 45 76 74 4d 61 6e 5c 50 61 72 61 6d 65 74 65 72 73 } //HKLM\SYSTEM\CurrentControlSet\Services\MPSEvtMan\Parameters  1
		$a_80_2 = {2f 76 20 22 53 65 72 76 69 63 65 4d 61 69 6e 22 } ///v "ServiceMain"  1
		$a_80_3 = {2f 74 20 52 45 47 5f 53 5a 20 2f 64 } ///t REG_SZ /d  1
		$a_00_4 = {20 00 2f 00 66 00 } //1  /f
		$a_00_5 = {39 00 39 00 31 00 61 00 61 00 35 00 38 00 64 00 2d 00 38 00 39 00 31 00 63 00 2d 00 34 00 35 00 64 00 36 00 2d 00 38 00 63 00 63 00 30 00 2d 00 35 00 33 00 65 00 64 00 64 00 33 00 61 00 66 00 37 00 39 00 32 00 65 00 } //-1 991aa58d-891c-45d6-8cc0-53edd3af792e
	condition:
		((#a_80_0  & 1)*1+(#a_80_1  & 1)*1+(#a_80_2  & 1)*1+(#a_80_3  & 1)*1+(#a_00_4  & 1)*1+(#a_00_5  & 1)*-1) >=5
 
}
rule Trojan_Win32_SusRegistryModification_H_2{
	meta:
		description = "Trojan:Win32/SusRegistryModification.H,SIGNATURE_TYPE_CMDHSTR_EXT,06 00 06 00 06 00 00 "
		
	strings :
		$a_80_0 = {72 65 67 2e 65 78 65 20 61 64 64 } //reg.exe add  1
		$a_80_1 = {48 4b 4c 4d 5c 53 4f 46 54 57 41 52 45 5c 4d 69 63 72 6f 73 6f 66 74 5c 57 69 6e 64 6f 77 73 5c 43 75 72 72 65 6e 74 56 65 72 73 69 6f 6e 5c 50 6f 6c 69 63 69 65 73 5c 53 79 73 74 65 6d } //HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System  1
		$a_80_2 = {2f 76 20 45 6e 61 62 6c 65 4c 69 6e 6b 65 64 43 6f 6e 6e 65 63 74 69 6f 6e 73 } ///v EnableLinkedConnections  1
		$a_00_3 = {2f 00 64 00 } //1 /d
		$a_80_4 = {2f 74 20 52 45 47 5f 44 57 4f 52 44 } ///t REG_DWORD  1
		$a_00_5 = {20 00 2f 00 66 00 } //1  /f
	condition:
		((#a_80_0  & 1)*1+(#a_80_1  & 1)*1+(#a_80_2  & 1)*1+(#a_00_3  & 1)*1+(#a_80_4  & 1)*1+(#a_00_5  & 1)*1) >=6
 
}